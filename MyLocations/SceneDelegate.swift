...
//MARK: - HELPER METHODS
    //This method is for handling errors that occur when a tagged location is saved or not saved
    
    //First section wants to be informed when a data is saved
    func listenForFatalCoreDataNotifications() {
        NotificationCenter.default.addObserver(forName: dataSaveFailedNotification, object: nil, queue: OperationQueue.main) {_ in
            ...
            let action = UIAlertAction(title: "0K", style: .default) {_ in let exception = NSException(name: NSExceptionName.internalInconsistencyException, reason: "Fatal Core Data error", userInfo: nil)
                exception.raise()
            }
